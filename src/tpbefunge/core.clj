...
(def random (Random.))
(def max-iter 200000)    ;; Para evitar bucles infinitos
           ...
           (if en-comillas?
             ...
             (case instr
                   ...
                   \p (let [[y p1] (desapilar pila)
                            [x p2] (desapilar p1)
                            [v p3] (desapilar p2)]
                           ;(assoc mapa-estado :grilla (assoc-in grilla [y x] (char v))
                   ...
                   \g (let [[y p1] (desapilar pila)
                            [x p2] (desapilar p1)]
                           (assoc mapa-estado :pila (apilar p2 (if(or
                                                                    (> x (- (:x dimensiones) 1))
                                                                    (> y (- (:y dimensiones) 1)))
                                                                 0
...
(defn ejecutar [filename]
      (let [mapa-estado (crear-mapa-estado filename)]
           (loop [iter 0
                  estado mapa-estado]
